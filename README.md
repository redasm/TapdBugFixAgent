...
Agent 工具调用总次数和同一工具次数均不设置固定上限，`find`、`read`、`grep` 等可按调查需要反复使用；只由各阶段总超时兜底。只读调查最长使用 10 分钟搜索，随后用最多 3 分钟根据已有轨迹强制收敛；超时或结构化输出不完整会自动重试，不会误标为 `needs_info`。**调查阶段没有任何转人工的出口**：`repair_contract.open_questions` 里的业务未决问题、`blocked_reasons` 里的「无法根据标题/描述/现有代码定位入口」以及「当前代码疑似已包含该修复、修复前基线不可确认」，都统一登记为 `调查证据尚未收敛: …` 校验缺项——先在同一次只读调查内做一轮定向补充核查，仍未收敛则按普通失败自动重试，重试耗尽后 `failed`；它们不消耗「人工补充信息」通道，也不需要用户补充工单内容。调查提示明确要求 Agent 依据工单与源码给出最合理且可验证的判断：能从源码、配置或上下文推断的不得交人，确实不确定时如实登记证据缺口供自动补查，且不得因代码疑似已修复而要求人工确认。无法运行游戏/编辑器/自动化等纯验证限制写入外层 `verification_limitations`，既不阻断修复，也会如实进入实施提示、评审提示、changelist 描述与审计证据，绝不当成“已验证”。

调查阶段仍会转 `blocked_workspace` 的只有四类明确的工作区/权限结论（解析器 `isWorkspaceSafetyReason` 与调查提示词共用同一套措辞）：目标修改路径/仓库不在允许访问范围、工作区或目标仓库缺权限而无法读写、无法在当前工作区安全修改、无法安全选择目标仓库或修改位置；另有 MCP/工作区基础设施故障。它们不消耗修复尝试次数，交人工处理。只说「某符号定义在 project 工作区之外的同名模块中」「无法读取工作区之外的外部依赖版本」「未确认该改动是否存在安全风险」或「Bug 所属仓库无法判断」都是证据缺口，会被判成 `调查证据尚未收敛` 走补查与自动重试，不会误判成工作区阻塞；**安全阻塞与普通缺口同时出现时两者都保留**——阻塞照旧交人工，缺口留在 `validation_errors` 并进入失败原因，绝不静默丢掉。`reproduction.before` 只接受实际观察到的失败或等价静态证据：「修复前基线不可确认」这类原文既不会让调查判为收敛（仍走定向补查与自动重试），也不会被当成修复前失败现象喂给实施 Agent，更不允许为了通过格式检查编造 before。必须修改但 MCP/权限无法安全处理的资源仍在实施阶段走 `manual_assets` / `manual_only` / `candidate_partial`。
...
任务列表「处理中 N」分组标题栏右侧跟随当前处理中的 bug（SSE 每 2 秒推送），展示**当前阶段**、
**这一刻正在跑的角色**与**该角色的模型**
（`/api/status` 的 `current_stage`：`current_stage` / `current_stage_label` / `current_role` / `current_model`）。
模型解析与真正调用 pi 时同源（`agents.roles.<role>.model || pi.provider` 默认模型）：
调查 / 实施阶段的 recovery 收尾子调用期间会同步切到 `recovery` 角色及其模型（阶段标签不变），
coordinator 的两次只读调用期间切到 `coordinator`，跑完随即还原；
准备、准入、机器验证等不调用模型的阶段 `current_role` 为 `null`、`current_model` 为空串，前端显示「—」；
没有处理中任务时该分组及其右侧状态整块不渲染。
日志只由服务端 PowerShell 输出，页面不再重复渲染日志面板。

**模型展示口径（三个字段各司其职，互不冒充）**：

- `job.model` / `default_model`：**默认回退模型** = `pi.provider` 的模型，即「没配
  `agents.roles.<role>.model` 的角色会回落到哪个模型」。它**不代表任何一次调用真的用过它**，
  设置页与详情页都按这个措辞展示（设置页另有 `pi.agent_roles` 角色模型配置摘要）。
- `job.role_models`：后端解析后的「角色 → 该角色实际会用的模型」摘要（含未配置的角色与
  `uses_default_model` 标记），用于说明配置口径。
- `detail.actual_models`：**本次任务真正调用过的** role/model 列表（`[{role, model, calls}]`），
  只从审计的 `agent_input` 事件推导，可含多个角色/多个模型；没有任何 `agent_input` 记录时为空数组，
  前端显示「未记录（本次审计没有 agent_input 事件）」，绝不用默认回退模型顶替。
...
- **可审计**：`agent_input` / `agent_usage` 审计事件带 `role`、解析后的 `model`、`timeout_s`、
  ...
  审计只落 prompt 的哈希，**不落 prompt 明文**。尝试元数据里另有 `agent_roles` 覆盖快照与
  `default_model`（= `pi.provider` 的默认回退模型；**不是**本次实际调用的模型，
  实际调用过的 role/model 只以 `agent_input` 为准，见管理台一章的 `actual_models`）。
...
> **必须写在 `agents.roles.coordinator`**：写在 `agents.coordinator`（少一层 `roles`）不会被解析，
> 属于静默失效——因此 `parseAgentsConfig` 会对 `agents` 下除 `roles` 外的未知键直接告警并提示
> 改写成 `agents.roles.coordinator`（启动日志的 `[配置警告]` 与会话内设置页都能看到）。
...
pending → in_progress
  ...
  ├─ needs_info / manual_only   # 仅准入层：描述过短/缺少复现信号 → needs_info；资源类人工处理 → manual_only（旧调查出口误标的 needs_info 启动迁移会自动恢复为 pending）
  ├─ manual_review             # 候选未完成：保留已有 changelist 和具体失败原因
  ├─ blocked_workspace         # 工作区/权限阻塞：残留无法归属的打开文件，或只读调查明确报告四类工作区/权限结论（目标修改路径/仓库不在允许访问范围、工作区或目标仓库缺权限、无法在当前工作区安全修改、无法安全选择目标仓库/修改位置）；处理/确认后人工重试
...
开发阶段只维护当前协议，不保留旧接口、旧字段兼容分支或旧数据库 schema 自动迁移；旧版本误判造成的任务状态恢复（`blocked_workspace` 里的 provider 故障、旧调查出口误标的 `needs_info`）属于进程启动对账，走上表列出的两类精确恢复方法，不直接改数据库。
...
- **旧 needs_info 的迁移**（`worker.ts` `recoverLegacyInvestigationNeedsInfo`）：旧版本把「业务条件尚未确认 / 基线不可确认（疑似已含修复）/ 无法定位入口」写成 `needs_info` 终态，新版本这三类都属于「调查未收敛」。启动时（`runLoop` 与 `runBatch` 两条路径都会调用）按旧固定外壳精确识别并放回 `pending`，走定向补查 + 普通自动重试。迁移只认旧外壳（`只读 Agent 明确无法定位问题: …`，或 `只读调查无法在无人工确认的情况下继续（不消耗修复尝试次数）: …` 且正文带业务/基线前缀），并要求 `attempts=0`、无 changelist、workspace 仍在配置中且仍有仓库可修；幂等，每次启动最多迁移 5 个（剩余任务后续启动继续迁移），不解除 provider 全局冷却，恢复前把原失败原因写进事件留痕。准入层（描述过短/缺少复现信号/资源关键词）的 `needs_info` 原因文本完全不同，绝不被迁移。
