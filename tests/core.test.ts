...
import { buildFixBranchName, GitWorkspace, GitWorkspaceError } from "../src/git.js";
...
function makeBug(over: Record<string, unknown> = {}): Bug {
  const data: Record<string, unknown> = {
    id: "1123456780001234007", // 字符串：>2^53 的大整数，Number 字面量会丢精度
...
describe("descgen", () => {
  ...
  it("bugShortId：前缀不匹配（异常数据）→ 回退末 7 位去前导零", () => {
    expect(bugShortId({ id: "1123456780001234007", workspace_id: "1123456780" })).toBe("1234007");
    expect(bugShortId({ id: "99000123", workspace_id: "12345678" })).toBe("9000123"); // 末 7 位无前导零
...
describe("Pi 模型和执行守卫", () => {
  ...
  it("实施阶段连续只读调用会在首次写入前熔断", () => {
    const guard = new WriteProgressGuard(2);
    expect(guard.hasWritten).toBe(false);
    guard.observeTool("read", { path: "a.ts" });
    guard.observeTool("grep", { pattern: "Foo" });
    expect(() => guard.observeTool("read", { path: "b.ts" })).toThrow(/仍未修改文件/);
  });

  it("真实编辑解除首次落笔熔断，但 p4 edit 不算内容修改", () => {
    expect(isFileWriteToolCall("Bash", { command: "p4 edit Source/Foo.cpp" })).toBe(false);
    expect(isFileWriteToolCall("apply_patch", { patch: "..." })).toBe(true);
    expect(isFileWriteToolCall("filesystem_write_file", { path: "a.ts" })).toBe(true);
    expect(isFileWriteToolCall("update_plan", { plan: [] })).toBe(false);
    expect(isFileWriteToolCall("Bash", { command: "apply_patch < fix.patch" })).toBe(true);
    expect(isFileWriteToolCall("Bash", { command: "Set-Content a.ts fixed" })).toBe(true);
    expect(isFileWriteToolCall("Bash", { command: "rg Foo Source 2>&1" })).toBe(false);
    expect(isFileWriteToolCall("Bash", { command: "rg Foo Source >/dev/null 2>&1" })).toBe(false);
    expect(isFileWriteToolCall("Bash", { command: "rg Foo Source >NUL 2>&1" })).toBe(false);

    const guard = new WriteProgressGuard(2);
    guard.observeTool("read", { path: "a.ts" });
    guard.observeTool("Bash", { command: "p4 edit Source/Foo.cpp" });
    expect(guard.hasWritten).toBe(false);
    guard.observeTool("Edit", { path: "a.ts" });
    expect(guard.hasWritten).toBe(true);
    guard.observeTool("read", { path: "a.ts" });
    guard.observeTool("read", { path: "b.ts" });
...
describe("Git 附加工作区", () => {
  it("按主分支、作者和秒级时间生成分支名", () => {
    expect(buildFixBranchName(
      "branch_0.7.0", "developer", new Date(2026, 6, 7, 17, 17, 30),
    )).toBe("branch_0.7.0_developer20260707171730");
  });
  ...
  it("创建修复分支、本地提交并切回主分支", async () => {
    ...
    expect(result?.branch).toBe("branch_0.7.0_developer20260707171730");
  ...
  it("失败回滚只清理本次创建的分支", async () => {
    ...
    execFileSync("git", ["commit", "-m", "base"], { cwd: repo });
    const workspace = new GitWorkspace(repo);
    const session = await workspace.prepareBranch("main", "developer", new Date(2026, 0, 2, 3, 4, 5));
...
`,
      ...
      expect(cfg.pi.provider?.id).toBe("custom"); // 未覆盖字段保留
  ...
  it("applySettingsOverrides 直接改运行中 config（web POST 路径）", () => {
    const cfg = makeConfig();
    cfg.pi.provider = { id: "custom", base_url: "https://old", model_id: "m1" };
...
describe("state", () => {
  ...
  it("记录人工接受、修改、拒绝和 reopen，并计算真实准确率指标", () => {
    const store = new StateStore(":memory:");
    const accepted = makeBug({ id: "1123456780001234001" });
    const modified = makeBug({ id: "1123456780001234002" });
    const rejected = makeBug({ id: "1123456780001234003" });
...
describe("p4", () => {
  ...
  it("P4 ignore_paths 可忽略已跟踪目录和任意层级生成目录", () => {
    ...
    expect(p4PathIgnored("//project/branch/Source/Client/Config/Build.ini", ignored)).toBe(true);
    expect(p4PathIgnored("//project/branch/Source/Client/TypeScript/.github/agents/a.md", ignored)).toBe(true);
    expect(p4PathIgnored("//project/branch/Source/Client/Source/Feature.cpp", ignored)).toBe(false);
  });
  ...
  it("opened() 解析真实 p4 输出（#rev 与路径间无空格）", async () => {
    ...
    (client as unknown as { run: () => Promise<string> }).run = async () => [
      "//project/branch_0.7.0/a.ts#6 - edit default change (text)",
      "//project/branch_0.7.0/b.ts#9 - edit change 1234 (unicode)",
      "//project/branch_0.7.0/c.ts#1 - add default change (text)",
    ].join("\n");
    const o = await client.opened();
    expect(o).toHaveLength(3);
    expect(o[0]).toEqual({ depot: "//project/branch_0.7.0/a.ts", action: "edit", changelist: "default", type: "text" });
    expect(o[1]).toEqual({ depot: "//project/branch_0.7.0/b.ts", action: "edit", changelist: "1234", type: "unicode" });
  ...
  it("opened() 排除 P4 ignore_paths 中已受控的文件", async () => {
    ...
    (client as unknown as { run: () => Promise<string> }).run = async () => [
      "//project/branch/Source/Client/Config/Build.ini#1 - edit default change (text)",
      "//project/branch/Source/Client/Source/Fix.cpp#2 - edit default change (text)",
    ].join("\n");
    expect(await client.opened("default")).toEqual([{
      depot: "//project/branch/Source/Client/Source/Fix.cpp",
      action: "edit",
      changelist: "default",
      type: "text",
    }]);
  ...
  it("取消后的结束清点客户端不继承取消信号，并保留 P4 环境与忽略规则", async () => {
    const cancel = new CancelEvent();
    cancel.set();
    const client = new P4Client(
      "C:\\tmp",
      { client: "test-client", port: "ssl:test:1666" },
      undefined,
      cancel,
      ["Config/"],
    );
    const cleanup = client.forkForCleanup();
    const run = vi.spyOn(P4Client.prototype, "run").mockImplementation(async function () {
      const signal = (this as unknown as { cancelSignal?: { cancelled: boolean } }).cancelSignal;
      if (signal?.cancelled) throw new P4CancelledError("人工取消 P4");
      return [
        "//depot/Config/Generated.ini#1 - edit default change (text)",
        "//depot/Fix.ts#2 - edit default change (text)",
      ].join("\n");
    });

    await expect(client.opened("default", true)).rejects.toBeInstanceOf(P4CancelledError);
    await expect(cleanup.opened("default", true)).resolves.toEqual([{
      depot: "//depot/Fix.ts",
      action: "edit",
      changelist: "default",
      type: "text",
    }]);
    expect(cleanup.path).toBe(client.path);
    expect(cleanup.env.P4CLIENT).toBe("test-client");
    expect(cleanup.env.P4PORT).toBe("ssl:test:1666");
  ...
  it("reconcile 过滤忽略路径，并只打开预览中允许的文件", async () => {
    ...
    (client as unknown as { run: (args: string[]) => Promise<string> }).run = async (args: string[]) => {
      calls.push(args);
      return [
        "//project/branch/Source/Client/Config/Build.ini#1 - opened for edit",
        "... //project/branch/Source/Client/Config/Build.ini - also opened by other@client",
        "//project/branch/Source/Client/TypeScript/.github/agents/a.md#1 - opened for add",
        "//project/branch/Source/Client/Source/Fix.cpp#2 - opened for edit",
      ].join("\n");
    };
    ...
    expect(calls[1]).toEqual(["reconcile", "//project/branch/Source/Client/Source/Fix.cpp"]);
  });
...
class FakeP4 {
  ...
  async reconcile(): Promise<string> {
    ...
    if (this.preview.trim() && !this._opened.length) {
      this._opened = [
        { depot: "//project/.../x.cpp", action: "edit", changelist: "default", type: "text" },
      ];
    }
    return "";
  }
...
describe("checkAndPrepareP4", () => {
  ...
  it("回归：只收集 default changelist 的文件，绝不混入编号 changelist（其它 bug 的 pending CL）", async () => {
    const fake = new FakeP4(
      [
        { depot: "//project/.../mine.ts", action: "edit", changelist: "default", type: "text" },
        { depot: "//project/.../other1.ts", action: "edit", changelist: "737633", type: "text" },
        { depot: "//project/.../other2.ts", action: "edit", changelist: "737633", type: "text" },
      ],
      "",
    );
    const opened = await checkAndPrepareP4(fake as unknown as P4Client);
    expect(opened.map((o) => o.depot)).toEqual(["//project/.../mine.ts"]);
  });

  it("回归：default 无改动但编号 changelist 有文件时，报错信息点明编号 changelist", async () => {
    const fake = new FakeP4(
      [{ depot: "//project/.../other.ts", action: "edit", changelist: "737633", type: "text" }],
      "",
    );
...
describe("agent parsing", () => {
  ...
  it("Pi provider 报错时不从 user prompt 回显解析 FINAL_RESULT 示例", () => {
    ...
    expect(extractFinalText(lines)).toBe("");
    expect(extractPiProviderError(lines)).toBe("Connection error.");
    expect(resultFromOutput(extractFinalText(lines), 1).changed_files).toEqual([]);
  });

  it("Pi provider 自动重试后成功时忽略早先的错误事件", () => {
    const lines = [
      JSON.stringify({
        type: "agent_end",
        messages: [{ role: "assistant", content: [], stopReason: "error", errorMessage: "temporary" }],
      }),
      JSON.stringify({ type: "auto_retry_start", attempt: 1, errorMessage: "temporary" }),
      JSON.stringify({
        type: "agent_end",
    ...
    expect(extractPiProviderError(lines)).toBeUndefined();
    expect(extractFinalText(lines)).toBe("ok");
  });
...
describe("PiAgent 子进程控制", () => {
  it("调查收尾显式空工具列表禁用全部 Pi 工具", async () => {
    const d = tmpdir();
    const script = path.join(d, "args.cjs");
  ...
  it("恢复轨迹保留多条完整证据，不混入用户示例和巨型增量事件", () => {
    const trace = piRecoveryTrace([
      { type: "message_end", message: { role: "user", content: [{ type: "text", text: "FINAL_RESULT: 用户示例" }] } },
      { type: "tool_execution_start", toolName: "read", args: { path: "Map.ts" } },
      { type: "tool_execution_end", toolName: "read", result: { content: [{ type: "text", text: "代码证据" }] } },
      { type: "message_end", message: { role: "assistant", content: [{ type: "text", text: "已定位空值分支" }] } },
      { type: "message_update", message: { content: "x".repeat(64000) } },
    ].map((event) => JSON.stringify(event)));
    expect(trace).toContain("Map.ts");
    expect(trace).toContain("代码证据");
    expect(trace).toContain("已定位空值分支");
    expect(trace).not.toContain("用户示例");
    expect(trace.length).toBeLessThan(1000);
  });
  it("Pi 进程 exit=0 但 provider stopReason=error 时按基础设施失败处理", async () => {
    const d = tmpdir();
    const script = path.join(d, "provider-error.cjs");
    ...
    writeFakePi(d, `node "${script}"`);
    const agent = new PiAgent(makeConfig());

    await withFakePiOnPath(d, async () => {
      await expect(agent.run({ prompt: "x", repoDir: d, timeoutS: 60 }))
        .rejects.toThrow(/max_tokens 参数非法/);
    });
  });
  ...
  it("显式传空 MCP 列表时不向普通代码 Bug 注入任何 MCP", async () => {
    const d = tmpdir();
    writeFakePi(d, "echo %* > args.txt & node -e \"console.log('done')\"");
    const cfg = makeConfig();
    cfg.mcp_servers = parseMcpServers({
      chrome_devtools: { command: "node", cwd: "{repo}", read_only_tools: ["list_pages"] },
      unreal_mcp: { command: "python", cwd: "{repo}", read_only_tools: ["ping"] },
    });
    const agent = new PiAgent(cfg);
    await withFakePiOnPath(d, async () => {
      await agent.run({
  ...
  it("只读阶段通过 --tools 限制为代码浏览工具，并可覆盖评审模型", async () => {
    ...
    expect(args).toContain("custom/reviewer-model");
  });
...
describe("worker web 合并", () => {
  it("列表包含未处理 bug", async () => {
    const w = makeWorker();
    stubMyBugs(w, [makeBug({ id: "1123456780001254287" })]);
  ...
  it("未处理详情字段齐全", async () => {
    const w = makeWorker();
    const bug = makeBug({ id: "1123456780001254287", description: "锻造引导后轮盘不显示" });
    stubMyBugs(w, [bug]);
  ...
  it("不在列表时按 id 直拉详情", async () => {
    const w = makeWorker();
    const bug = makeBug({ id: "1123456780001254287", description: "直接按 id 拉取" });
    stubMyBugs(w, []);
...
describe("本地任务（Tapd 列表外）可见可处理", () => {
  ...
  it("快照 tapd_status 已终态（resolved）的本地 pending 不复活", async () => {
    const w = makeWorker([{ name: "r", path: "C:\\tmp", verify_cmds: [] }]);
    const bug = makeBug({ id: "1123456780001254287", status: "resolved" });
    stubMyBugs(w, []);
    stubTapd(w, new FakeTapd([bug]));
  ...
  it("retryAllFailed 重置全部失败任务，非失败状态不动", async () => {
    const w = makeWorker();
    const a = makeBug({ id: "1123456780001254287" });
    const b = makeBug({ id: "1123456780001254288" });
    const ok = makeBug({ id: "1123456780001254289" });
  ...
  it("resyncFromTapd 保留候选/人工结论并按最新 Tapd 列表重建可重试队列", async () => {
    const w = makeWorker();
    const live1 = makeBug({ id: "1123456780001254287" }); // Tapd 上还在
    const live2 = makeBug({ id: "1123456780001254288", status: "resolved" }); // Tapd 终态 → 不同步
    const gone = makeBug({ id: "1123456780001254290" }); // 只有本地有（Tapd 列表外）→ 应被清掉
  ...
  it("停止后当前任务尚未退出时拒绝清除同步，避免删除运行中任务的状态和日志", async () => {
    const w = makeWorker();
    w.store.setControl("stopped");
    w.currentBugId = "1123456780001254287";
...
describe("worker 控制与取消", () => {
  ...
  it("进程启动对账：遗留 in_progress 回退为 pending", async () => {
    ...
    w.store.upsertJob(makeBug({ id: "1123456780001254289" }), { agent_state: "skipped" });
    ...
    expect(w.store.getJob("1123456780001254289")?.agent_state).toBe("skipped");
    const ev = w.store.listEvents(a.id);
    expect(ev.some((e) => String(e.msg).includes("对账"))).toBe(true);
  });

  it("进程启动对账：旧版本因 Agent 超时或工具预算误标的 needs_info 恢复为 pending", () => {
    const w = makeWorker();
    const timedOut = makeBug({ id: "1123456780001254290" });
    const trulyBlocked = makeBug({ id: "1123456780001254291" });
    const limited = makeBug({ id: "1123456780001254292" });
    w.store.upsertJob(timedOut, {
      agent_state: "needs_info",
      failure_reason: "Agent 调用超时(1800s): pi",
      finished_at: "2026-09-03 21:00:00",
    });
    w.store.upsertJob(trulyBlocked, {
      agent_state: "needs_info",
      failure_reason: "只读 Agent 明确无法定位问题",
    });
    w.store.upsertJob(limited, {
      agent_state: "needs_info",
      failure_reason: "Agent 命令调用超过预算 80 次，已停止继续执行",
      finished_at: "2026-09-04 19:09:28",
    });

    (w as unknown as { reconcileStaleInProgress(): void }).reconcileStaleInProgress();

    expect(w.store.getJob(timedOut.id)?.agent_state).toBe("pending");
    expect(w.store.getJob(timedOut.id)?.failure_reason).toBeNull();
    expect(w.store.getJob(trulyBlocked.id)?.agent_state).toBe("needs_info");
    expect(w.store.getJob(limited.id)?.agent_state).toBe("pending");
...
describe("formatRetryEvidence", () => {
  ...
  it("超时重试要求直接收敛，避免再次耗尽整轮预算", () => {
    const text = formatRetryEvidence([{
      attempt: 1,
      at: "2026-09-04 18:11:41",
      failure_reason: "Error: Agent 调用超时(1800s): pi",
      opened_files: [],
      agent_summary: "",
      manual_assets: [],
    }]);
...
describe("worker 两阶段修复协议", () => {
  it("专用 P4 工作区正常完成后只做一次启动基线扫描", async () => {
    ...
    const first = makeBug({ id: "1123456780001234007" });
    const second = makeBug({ id: "1123456780001234008" });
  ...
  it("同一 Bug 可只修改 Git 引擎目录并保留本地分支和 commit", async () => {
    ...
    vi.spyOn(GitWorkspace.prototype, "prepareBranch").mockResolvedValue({
      baseBranch: "branch_0.7.0", baseCommit: "base", branch: "branch_0.7.0_developer20260707171730",
    });
    ...
    vi.spyOn(GitWorkspace.prototype, "finalize").mockResolvedValue({
      branch: "branch_0.7.0_developer20260707171730",
    ...
    expect(comments.join("\n")).toContain("branch_0.7.0_developer20260707171730");
  ...
  it("Git 工作区已有改动只进入阻塞，不消耗模型重试", async () => {
    const w = makeWorker([{
      name: "game", path: "C:/project", verify_cmds: [],
      additional_dirs: [{ name: "engine", path: "C:/engine", vcs: "git", base_branch: "main", author: "me", verify_cmds: [] }],
    }]);
    const bug = makeBug();
    vi.spyOn(P4Client.prototype, "opened").mockResolvedValue([]);
    const run = vi.spyOn(PiAgent.prototype, "run").mockResolvedValue(makeInvestigation("engine:Engine/Foo.cpp"));
    vi.spyOn(GitWorkspace.prototype, "prepareBranch").mockRejectedValue(new GitWorkspaceError("Git 工作区存在未提交改动"));
    await w.processBug(bug);
    expect(run).toHaveBeenCalledOnce();
    expect(w.store.getJob(bug.id)?.agent_state).toBe("blocked_workspace");
    expect(w.store.getJob(bug.id)?.attempts).toBe(0);
  });

  it("验证同时登记预打开文件以外的实际改动，再准确拒绝越界", async () => {
    const w = makeWorker();
    const p4 = new P4Client("C:/project");
    const opened: OpenedFile[] = [{ depot: "//depot/Plan.ts", action: "edit", changelist: "default", type: "text" }];
    const preview = vi.spyOn(p4, "reconcilePreview").mockResolvedValue("//depot/Actual.ts#1 - reconcile to edit");
    const reconcile = vi.spyOn(p4, "reconcile").mockResolvedValue("");
    vi.spyOn(p4, "opened").mockResolvedValue([...opened, { depot: "//depot/Actual.ts", action: "edit", changelist: "default", type: "text" }]);
    const verify = (w as unknown as { verifyCandidate: (...args: unknown[]) => Promise<unknown> }).verifyCandidate.bind(w);
    await expect(verify(p4, { path: "C:/project", verify_cmds: [] }, [], opened,
      ["project:Plan.ts"], ["project:Actual.ts"])).rejects.toThrow("实际修改超出调查阶段计划范围");
    expect(preview).toHaveBeenCalledWith(["./Plan.ts", "./Actual.ts"]);
  ...
  it("跨目录修复失败时回滚本次 Git 分支", async () => {
    const repo: RepoConfig = {
      ...
      additional_dirs: [{
        ...
        base_branch: "main", author: "developer", verify_cmds: [],
      }],
    };
    const w = makeWorker([repo]);
    const bug = makeBug();
    ...
    vi.spyOn(P4Client.prototype, "reconcilePreview").mockResolvedValue("");
    vi.spyOn(GitWorkspace.prototype, "prepareBranch").mockResolvedValue({
      baseBranch: "main", baseCommit: "base", branch: "main_developer20260707171730",
    });
  ...
  it("编码阶段 900 秒未落笔时保留 1800 秒总上限，并只做一次 180 秒定向收尾", async () => {
    ...
    vi.spyOn(PiAgent.prototype, "run").mockImplementation(async (opts) => {
      ...
      if (calls.length === 2) {
        throw new AgentInvestigationLimitError(
          "实施阶段 900s 内仍未产生文件写入，已停止无效停滞",
          "已确认 Login.ts 中空值分支需要修改",
        );
      }
      return makeResult({ changed_files: ["Login.ts"], summary: "定向完成修复" });
    });
    vi.spyOn(P4Client.prototype, "sync").mockResolvedValue("");
    let openedCalls = 0;
    vi.spyOn(P4Client.prototype, "opened").mockImplementation(async () => {
      openedCalls += 1;
      return openedCalls >= 3
        ? [{ depot: "//depot/Login.ts", action: "edit", changelist: "default", type: "text" }]
        : [];
    });
    vi.spyOn(P4Client.prototype, "reconcilePreview").mockResolvedValue("");
    ...
    expect(calls).toHaveLength(3);
    expect(calls[1].timeoutS).toBe(1800);
    expect(calls[1].maxSecondsBeforeWrite).toBe(900);
    expect(calls[2].timeoutS).toBe(180);
    expect(calls[2].maxSecondsBeforeWrite).toBe(90);
    expect(calls[2].maxCommandExecutions).toBe(Number.POSITIVE_INFINITY);
    expect(calls[2].maxReadOnlyExecutionsBeforeWrite).toBe(Number.POSITIVE_INFINITY);
  ...
  it.each(["timeout", "limit"])("%s 时 P4 只有预打开文件必须定向恢复，不能当成真实补丁", async (kind) => {
    const repo = tmpdir();
    fs.writeFileSync(path.join(repo, "Login.ts"), "old");
    const w = makeWorker([{ name: "r", path: repo, verify_cmds: [] }]);
    const bug = makeBug();
    ...
    vi.spyOn(PiAgent.prototype, "run").mockImplementation(async (opts) => {
      ...
      if (calls.length === 1) return makeInvestigation("Login.ts");
      if (calls.length === 2) {
        const ErrorType = kind === "timeout" ? AgentTimeoutError : AgentInvestigationLimitError;
        throw new ErrorType("预算耗尽", "已确认 Login.ts 的空值分支");
      }
      changed = true;
      return makeResult({ changed_files: ["project:Login.ts"], summary: "完成修复" });
    });
    vi.spyOn(P4Client.prototype, "sync").mockResolvedValue("");
    vi.spyOn(P4Client.prototype, "edit").mockImplementation(async () => { opened = true; return ""; });
    const revert = vi.spyOn(P4Client.prototype, "revertUnchanged").mockImplementation(async () => {
      if (!changed) opened = false;
      return "";
    });
    vi.spyOn(P4Client.prototype, "opened").mockImplementation(async () => opened
      ? [{ depot: "//depot/Login.ts", action: "edit", changelist: "default", type: "text" }] : []);
    vi.spyOn(P4Client.prototype, "reconcilePreview").mockImplementation(async () => changed && !opened
      ? "//depot/Login.ts#1 - reconcile to edit" : "");
    vi.spyOn(P4Client.prototype, "reconcile").mockImplementation(async () => { opened = true; return ""; });
    vi.spyOn(P4Client.prototype, "diffUnified").mockImplementation(async () => changed
      ? "--- a/Login.ts\n+++ b/Login.ts\n-old\n+fixed" : "==== //depot/Login.ts#1 - Login.ts ====\n");
    vi.spyOn(P4Client.prototype, "createPending").mockResolvedValue(4321);

    await w.processBug(bug);

    expect(calls).toHaveLength(3);
    expect(String(calls[2].prompt)).toContain("编码阶段已进入强制收尾");
    expect(revert).toHaveBeenCalled();
    expect(w.store.getJob(bug.id)?.agent_state).toBe("candidate");
  });

  it.each([true, false])("超时补丁验证后必须继续独立评审（approved=%s），不因缺失收尾而重做", async (approved) => {
    const repo = tmpdir();
    fs.writeFileSync(path.join(repo, "Login.ts"), "old");
    const w = makeWorker([{ name: "r", path: repo, verify_cmds: ['node -e "process.exit(0)"'] }]);
    ...
    vi.spyOn(P4Client.prototype, "sync").mockResolvedValue("");
    vi.spyOn(P4Client.prototype, "edit").mockResolvedValue("");
    vi.spyOn(P4Client.prototype, "opened").mockResolvedValueOnce([])
      .mockResolvedValue([{ depot: "//depot/Login.ts", action: "edit", changelist: "default", type: "text" }]);
    vi.spyOn(P4Client.prototype, "revertUnchanged").mockResolvedValue("");
    vi.spyOn(P4Client.prototype, "reconcilePreview").mockResolvedValue("");
    vi.spyOn(P4Client.prototype, "diffUnified").mockResolvedValue("--- a/Login.ts\n+++ b/Login.ts\n-old\n+fixed");
    ...
    expect(run).toHaveBeenCalledTimes(3);
    expect(w.store.getJob(bug.id)?.agent_state).toBe(approved ? "review_pending" : "pending");
    if (approved) expect(createPending).toHaveBeenCalledOnce();
    else expect(createPending).not.toHaveBeenCalled();
  });
  ...
  it("Agent 超时后强制收敛，仍失败则自动重试而不是 needs_info", async () => {
    ...
    expect(w.store.getJob(bug.id)?.agent_state).toBe("pending");
    expect(Number(w.store.getJob(bug.id)?.attempts ?? 0)).toBe(1);
    expect(String(w.store.getJob(bug.id)?.failure_reason)).toContain("超时");
  });

  it("调查达到工具预算后用已有轨迹强制收敛，不误标 needs_info", async () => {
    const w = makeWorker([{ name: "r", path: "C:\\tmp", verify_cmds: [] }]);
    const bug = makeBug();
    stubTapd(w, { addComment: async () => {}, updateBug: async () => {} } as unknown as FakeTapd);
    const calls: Array<Record<string, unknown>> = [];
    vi.spyOn(PiAgent.prototype, "run").mockImplementation(async (opts) => {
      ...
      if (calls.length === 1) {
        throw new AgentInvestigationLimitError(
          "Agent 命令调用超过预算 48 次，已停止继续执行",
          "已定位 MapMarkChooseView.ts 与 UIExtendToggle Hover 配置",
        );
      }
      if (calls.length === 2) return makeInvestigation("project:MapMarkChooseView.ts");
  ...
  it("Agent 修改 planned_files 之外的文件时拒绝自动候选，但保留到人工评审 changelist", async () => {
    ...
    expect(w.store.getJob(bug.id)?.agent_state).toBe("manual_review");
    expect(w.store.getJob(bug.id)?.changelist).toBe(4322);
    expect(w.store.getJob(bug.id)?.last_attempt_files).toBeNull();
  ...
  it("准入不通过时不调用 Agent，并记录需要补充的信息", async () => {
    ...
    const bug = makeBug({ name: "功能异常", description: "", module: "" });
    stubMyBugs(w, [bug]);
    stubTapd(w, { addComment: async () => {}, updateBug: async () => {} } as unknown as FakeTapd);
    const run = vi.spyOn(PiAgent.prototype, "run");
...
describe("worker 自动重试", () => {
  it("写入意图在进程退出前持久化，并可用相对路径匹配 P4 depot 文件", async () => {
    const w = makeWorker([{ name: "r", path: "C:\\repo", verify_cmds: [] }]);
    const bug = makeBug();
    w.store.upsertJob(bug, { agent_state: "in_progress" });
    const p4 = new P4Client("C:\\repo", {});
    const workerPrivate = w as unknown as {
      recordWriteIntent: (id: string, client: P4Client, planned: string[], file?: string) => void;
      cleanupStaleAttempt: (id: string, client: P4Client) => Promise<string[]>;
    };
    workerPrivate.recordWriteIntent(
      bug.id,
      p4,
      ["project:TypeScript/Src/Foo.ts"],
      "C:\\repo\\TypeScript\\Src\\Bar.ts",
    );
    expect(JSON.parse(String(w.store.getJob(bug.id)?.last_attempt_files))).toEqual([
      "TypeScript/Src/Foo.ts",
      "TypeScript/Src/Bar.ts",
    ]);
    vi.spyOn(P4Client.prototype, "opened").mockResolvedValue([
      { depot: "//depot/main/TypeScript/Src/Foo.ts", action: "edit", changelist: "default", type: "text" },
      { depot: "//depot/main/TypeScript/Src/Bar.ts", action: "edit", changelist: "default", type: "text" },
    ]);
    const revert = vi.spyOn(P4Client.prototype, "revert").mockResolvedValue("");

    await workerPrivate.cleanupStaleAttempt(bug.id, p4);

    expect(revert).toHaveBeenCalledWith([
      "//depot/main/TypeScript/Src/Foo.ts",
      "//depot/main/TypeScript/Src/Bar.ts",
    ]);
  });

  it("default 遗留文件可由写入事件唯一归属时自动隔离为人工评审", async () => {
    const w = makeWorker([{ name: "r", path: "C:\\repo", verify_cmds: [] }]);
    const owner = makeBug({ id: "1123456780001234991", name: "旧任务" });
    const current = makeBug({ id: "1123456780001234992", name: "当前任务" });
    w.store.upsertJob(owner, { agent_state: "pending" });
    w.store.upsertJob(current, { agent_state: "in_progress" });
    w.store.addEvent("Agent: edit C:\\repo\\TypeScript\\Src\\Foo.ts", "debug", owner.id);
    const p4 = new P4Client("C:\\repo", {});
    vi.spyOn(P4Client.prototype, "diffUnified").mockResolvedValue("--- old\n+++ new\n-old\n+new");
    const createPending = vi.spyOn(P4Client.prototype, "createPending").mockResolvedValue(9001);
    const preserve = (w as unknown as {
      preserveCrashOrphans: (client: P4Client, files: string[], currentId: string) => Promise<boolean>;
    }).preserveCrashOrphans;

    const handled = await preserve.call(
      w,
      p4,
      ["//depot/main/TypeScript/Src/Foo.ts"],
      current.id,
    ...
    expect(createPending).toHaveBeenCalledWith(
      expect.stringContaining("进程在写入后异常中断"),
      ["//depot/main/TypeScript/Src/Foo.ts"],
    );
    expect(w.store.getJob(owner.id)?.agent_state).toBe("manual_review");
    expect(w.store.getJob(owner.id)?.changelist).toBe(9001);
  });

  it("人工取消后仍登记本次 default 文件，下一次只精确撤销该文件", async () => {
    const w = makeWorker([{ name: "r", path: "C:\\tmp", verify_cmds: [] }]);
    const bug = makeBug({ id: "1123456780001260396" });
    w.store.upsertJob(bug, { agent_state: "in_progress" });
    const p4 = new P4Client("C:\\tmp", {}, undefined, w.cancelEvent);
    w.cancelEvent.set();
    const current = [
      { depot: "//depot/RecruitMinePanel.ts", action: "edit", changelist: "default", type: "text" },
      { depot: "//depot/OtherBug.ts", action: "edit", changelist: "737700", type: "text" },
    ];
    vi.spyOn(P4Client.prototype, "opened").mockImplementation(async function (cl?: string) {
      const signal = (this as unknown as { cancelSignal?: { cancelled: boolean } }).cancelSignal;
      if (signal?.cancelled) throw new P4CancelledError("人工取消 P4");
      return current.filter((o) => !cl || o.changelist === cl);
    });
    const revertCalls: Array<{ files: string[]; changelist?: string }> = [];
    vi.spyOn(P4Client.prototype, "revert").mockImplementation(async (files, changelist) => {
      revertCalls.push({ files, changelist });
      return "";
    });

    await (w as unknown as {
      recordAttemptEnd: (bugId: string, client: P4Client) => Promise<string[]>;
    }).recordAttemptEnd(bug.id, p4);
    expect(JSON.parse(String(w.store.getJob(bug.id)?.last_attempt_files))).toEqual([
      "//depot/RecruitMinePanel.ts",
    ]);

    const nextAttempt = p4.forkForCleanup();
    await (w as unknown as {
      cleanupStaleAttempt: (bugId: string, client: P4Client) => Promise<string[]>;
    }).cleanupStaleAttempt(bug.id, nextAttempt);
    expect(revertCalls).toEqual([{
      files: ["//depot/RecruitMinePanel.ts"],
      changelist: undefined,
    }]);
    expect(w.store.getJob(bug.id)?.last_attempt_files).toBeNull();
  });

  it("结束清点失败时不把已有遗留记录覆盖为空", async () => {
    const w = makeWorker([{ name: "r", path: "C:\\tmp", verify_cmds: [] }]);
    const bug = makeBug();
    w.store.upsertJob(bug, {
      last_attempt_files: dumps(["//depot/known.ts"]),
    });
...
describe("ensurePiModels", () => {
  ...
  it("max_tokens 超过 Pi 网关上限时自动钳制为 131072", () => {
    ...
    const model = JSON.parse(fs.readFileSync(file, "utf-8")).providers.custom.models[0];
  ...
  it("合并写入时保留已有其它 provider", () => {
    ...
    const pi: Config["pi"] = {
      provider: { id: "custom", base_url: "https://gateway", model_id: "m" },
    };
    ensurePiModels(pi, file);

    const root = JSON.parse(fs.readFileSync(file, "utf-8"));
    expect(root.providers.other.baseUrl).toBe("https://x");
    expect(root.providers.custom.baseUrl).toBe("https://gateway");
  });
  ...
  it("未配置 provider / 缺 base_url / 无 model_id 时不写文件", () => {
    ...
    ensurePiModels(
      { provider: { id: "custom", model_id: "m" } }, // 缺 base_url
      file,
    );
    expect(fs.existsSync(file)).toBe(false);

    ensurePiModels(
      { provider: { id: "custom", base_url: "https://g" } }, // 无 model_id
      file,
    );
    expect(fs.existsSync(file)).toBe(false);
  });
  ...
  it("损坏的原文件不炸：从空结构重建", () => {
    ...
    ensurePiModels(
      { provider: { id: "custom", base_url: "https://g", model_id: "m" } },
      file,
    );
    const root = JSON.parse(fs.readFileSync(file, "utf-8"));
    expect(root.providers.custom.baseUrl).toBe("https://g");
  });
});
...
describe("effectivePiModel", () => {
  ...
  it("拼成 <provider>/<model_id>", () => {
    expect(effectivePiModel({ provider: { id: "custom", model_id: "claude-opus-4-8" } }))
      .toBe("custom/claude-opus-4-8");
  });

  it("model_id 已带 '/' 时原样返回", () => {
    expect(effectivePiModel({ provider: { id: "custom", model_id: "custom/claude-opus-4-8" } }))
      .toBe("custom/claude-opus-4-8");
  });
});
...
describe("TapdMcpClient 首次调用可发现工具", () => {
  beforeEach(() => {
    ...
    mcpMockState.callResult = {
      content: [{ type: "text", text: JSON.stringify([{ id: "1001", title: "测试缺陷", workspace_id: "12345678", status: "new" }]) }],
    };
  });

  it("listBugs 首次调用（无前置 connect）就能匹配工具并解析列表", async () => {
    const client = new TapdMcpClient("12345678", { transport: "stdio" });
  ...
  it("通过 TAPD MCP 把描述图片路径解析为 300 秒临时 URL", async () => {
    mcpMockState.tools = [{
      name: "tapd_get_image_url",
      description: "获取图片 URL",
      inputSchema: {
        type: "object",
        properties: { workspace_id: { type: "number" }, image_path: { type: "string" } },
      },
    }];
    mcpMockState.callResult = {
      structuredContent: { result: { data: { download_url: "https://tapd.example/temp.png?expires=300" } } },
      content: [],
    } as never;
    const client = new TapdMcpClient("12345678", { transport: "stdio" });
    const bug = makeBug({ description: '<p>现象</p><img src="/tfl/captures/a.png">' });

    const media = await client.resolveMediaInputs(bug);

    expect(mcpMockState.calledTools).toContain("tapd_get_image_url");
    expect(media).toEqual([{ kind: "image", url: "https://tapd.example/temp.png?expires=300" }]);
  });
});
