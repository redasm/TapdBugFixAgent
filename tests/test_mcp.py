...
class TestTapdMcpE2E(unittest.TestCase):
    ...
    def test_list_bugs(self):
        ...
        self.assertEqual(ids, [1123456780001234007, 1123456780001234008])
        # 字段归一化
        first = bugs[0]
    ...
    def test_get_bug(self):
        bug = self.client.get_bug(1123456780001234008)
        self.assertEqual(bug.id, 1123456780001234008)
        self.assertEqual(bug.title, "配置表数值越界")

    def test_update_and_comment(self):
        res = self.client.update_bug(1123456780001234007, status="resolved")
        self.assertFalse(res["is_error"])
        self.assertEqual(res["data"]["status"], "resolved")
        # 内容参数正确映射到 description
        res2 = self.client.add_comment(1123456780001234007, "自动修复完成")
